...
GUILD_ID = 1531643603741442129
CATEGORY_ID = 1531709462635741297
SUPPORT_ROLE_ID = 1538332828645855287
LOG_CHANNEL_ID = 1538333206678343691
# ===================================================================
...
class TicketSelect(Select):
    def __init__(self):
        options = [
            discord.SelectOption(label="💻 Техническая поддержка", description="Проблемы с ботом, ошибки, баги", emoji="🛠️", value="tech"),
            discord.SelectOption(label="🤝 Сотрудничество", description="Партнёрство, реклама, отзывы", emoji="📢", value="partner"),
            discord.SelectOption(label="❓ Другое", description="Если не подходит ни один вариант", emoji="📝", value="other")
        ]
        ...
        try:
            ...
            for channel in guild.channels:
                if channel.name == f"ticket-{interaction.user.name}":
                    await interaction.response.send_message("❌ У вас уже есть открытый тикет! Закройте его, чтобы создать новый.", ephemeral=True)
                    return
            ...
            if not support_role:
                await interaction.response.send_message("❌ Ошибка: роль поддержки не найдена! Обратитесь к администратору.", ephemeral=True)
                return
            ...
            channel = await guild.create_text_channel(channel_name, category=category, overwrites=overwrites, reason=f"Тикет от {interaction.user.name}")

            ticket_types = {"tech": "💻 Техническая поддержка", "partner": "🤝 Сотрудничество", "other": "❓ Другое"}
            ticket_type = ticket_types.get(self.values[0], "❓ Другое")

            colors = {"tech": 0xFF4444, "partner": 0x4444FF, "other": 0xFFFF44}
            color = colors.get(self.values[0], 0x5865F2)

            embed = discord.Embed(title="🎫 Новый тикет", description=f"**Тип:** {ticket_type}", color=color, timestamp=datetime.now())
            ...
            close_button = Button(label="❌ Закрыть тикет", style=discord.ButtonStyle.danger, custom_id="close_ticket")
            claim_button = Button(label="👋 Взять тикет", style=discord.ButtonStyle.success, custom_id="claim_ticket")

            view = View()
            view.add_item(claim_button)
            view.add_item(close_button)

            await channel.send(f"{interaction.user.mention} {support_role.mention}", embed=embed, view=view)

            log_channel = guild.get_channel(LOG_CHANNEL_ID)
            if log_channel:
                log_embed = discord.Embed(title="📩 Новый тикет", description=f"**Канал:** {channel.mention}\n**Создал:** {interaction.user.mention}\n**Тип:** {ticket_type}", color=0x00FF00, timestamp=datetime.now())
            ...
            await interaction.response.send_message(f"❌ Ошибка при создании тикета: {str(e)[:100]}", ephemeral=True)
...
class TicketPanelView(View):
    ...
    @discord.ui.button(label="🎫 Создать тикет", style=discord.ButtonStyle.primary, custom_id="create_ticket", emoji="🎫")
    ...
    if interaction.type == discord.InteractionType.component:
        try:
            ...
            if custom_id == "close_ticket":
                ...
                if log_channel:
                    log_embed = discord.Embed(title="🔒 Тикет закрыт", description=f"**Канал:** {interaction.channel.mention}\n**Закрыл:** {interaction.user.mention}", color=0xFF0000, timestamp=datetime.now())
                    log_embed.set_footer(text="ArbuZ Support System")
                    await log_channel.send(embed=log_embed)
                ...
                if not support_role:
                    await interaction.response.send_message("❌ Роль поддержки не найдена!", ephemeral=True)
                    return
                ...
                if support_role in interaction.user.roles:
                    ...
                    log_channel = interaction.guild.get_channel(LOG_CHANNEL_ID)
                    if log_channel:
                        log_embed = discord.Embed(title="👋 Тикет взят", description=f"**Канал:** {interaction.channel.mention}\n**Сотрудник:** {interaction.user.mention}", color=0x00FF00, timestamp=datetime.now())
                        log_embed.set_footer(text="ArbuZ Support System")
                        await log_channel.send(embed=log_embed)
                else:
                    await interaction.response.send_message("❌ У вас нет прав для взятия тикета!", ephemeral=True)
...
async def ticket_panel(ctx):
    try:
        ...
        embed = discord.Embed(title="🎫 **ArbuZ Support System**", description=(
        ...
        embed.set_footer(text="ArbuZ Support System", icon_url=ctx.guild.icon.url if ctx.guild.icon else None)
...
async def get_ids(ctx):
    embed = discord.Embed(title="📋 ID на сервере", color=0x5865F2)
...
@bot.event
async def on_ready():
    await bot.change_presence(status=discord.Status.online, activity=discord.Activity(type=discord.ActivityType.watching, name="ArbuZ | !ticket_panel"))
...
async def start_bot():
    if not TOKEN:
        print("❌ ОШИБКА: Токен не найден! Добавь DISCORD_TOKEN в Environment на Render")
        return

    print("⏳ Ожидание 30 секунд перед подключением к Discord...")
    time.sleep(30)
    ...
    while retry_count < max_retries:
        try:
            ...
            if "429" in str(e) or "1015" in str(e):
                ...
                print(f"⚠️ Discord заблокировал IP (1015/429). Попытка {retry_count}/{max_retries}")
    ...
    print("🛑 Бот остановлен после 10 попыток.")
